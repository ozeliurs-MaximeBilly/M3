...
/* EXO5 */
CREATE OR REPLACE TRIGGER COEF
BEFORE INSERT OR UPDATE
ON MODULE
DECLARE
    TOTAL_TEST NUMBER(2,0);
    TOTAL_CC NUMBER(2,0);
BEGIN
    SELECT SUM(COEFF_TEST) INTO TOTAL_TEST FROM MODULE WHERE CODE <> :NEW.CODE;
    SELECT SUM(COEFF_CC) INTO TOTAL_CC FROM MODULE WHERE CODE <> :NEW.CODE;
    IF TOTAL_TEST+TOTAL_CC <> 100 THEN
        RAISE_APPLICATION_ERROR(-20003, "Coefficient ne correspondant pas !")
    END IF;
END;
